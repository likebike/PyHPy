...
# I intentionally split the string up and introduce a triple-quote before the 'inherit' line so that the mako_render.py parser doesn't think it's actually a dependency of this 'photos.html.mako' file.
'''<%inherit file="../_photoPage.mako" />
...
''').substitute(dict(this_photo=photo,
      ...
      photo_page_path = os.path.join(photo_pages_dir, '%s.html.mako'%(photo,))
...
<ul class=thumbnails>
%for p in photos:
  <li class=thumbnail><a href="${self.URL('/photos/%s.html'%(p,), mtime=None)}"><img src="${self.URL(pyhpy.thumb(self.FS_ROOT(), '/static/photoalbum/%s.jpeg'%(p,), width=162, height=100))}"></a></li>
%endfor
</ul>
