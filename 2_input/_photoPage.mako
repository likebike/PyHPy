...
<div id=bigPhotoArea>
  ...
  <a id=next class=navLink href="${self.URL('/photos/%s.html'%(self.NEXT_PHOTO(),), mtime=None)}">Next&nbsp;<i class="fa fa-arrow-right"></i></a>
  <%
    photoPath = os.path.join(self.photosDir(), self.THIS_PHOTO()) + '.jpeg'  # We chopped off the extension in THIS_PHOTO(), so we need to add it back on.
