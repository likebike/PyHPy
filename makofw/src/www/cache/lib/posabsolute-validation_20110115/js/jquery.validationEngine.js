...
(function($) {
	...
	$(this).bind("submit", function(caller){   // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
		...
		if($.validationEngine.submitValidation(this,settings) === false){
            ...
            caller.stopImmediatePropagation();              // BY CHRISTOPHER.  I decided I always want to do this on failure.
			settings.failure && settings.failure(caller);   // BY CHRISTOPHER, 2011-01-15: Added Caller Arg so I can cancel propagation.
			return false;
		}		
	});
...
$.validationEngine = {
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		function _required(caller,rules){   // VALIDATE BLANK FIELD
            if ($(caller).attr("disabled")==true) return;   //////////////////////////////////////// BY CHRISTOPHER SEBASTIAN.
			var callerType = $(caller).attr("type");
